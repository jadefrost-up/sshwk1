...
def get_PEHeader_features(path):
    peheader_features = [0 for i in range(len(HEADER))]
    try:
        pe = pefile.PE(path)
    except:
    ...
    tep1 = dir(pe)
    if 'NT_HEADERS' in tep1:
        tep2 = dir(pe.NT_HEADERS)
        if 'OPTIONAL_HEADER' in tep2:
            for i in range(len(HEADER)):
                peheader_features[i] = getattr(pe.NT_HEADERS.OPTIONAL_HEADER, HEADER[i])
        else:
            return peheader_features
    else:
        return peheader_features
